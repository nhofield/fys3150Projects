...
using namespace std;
double* LocalEnergyT2U(int N_P, int D, double** &pos, double psi_curr, double h_R, double alpha, double beta, double omega,
    ...
    for(double var_param = var_init; var_param <= var_final; var_param += h_var)
      {
        ...
        if( fix_specify == "alpha" )
            // Alpha fixed and beta varies
          {
            ...
            h_R = LocateOptimal_h_R(N_P, D, fix_value, var_param, omega, TrialWaveFunction2);
            ...
            MetropolisSampling(N_P, D, MCcycles, StoreValues, fix_value, var_param, omega, h_R,
              TrialWaveFunction2, LocalEnergyT2U, E_Local_Analytic_T2P);
        ...
        if( fix_specify == "beta")
          ...
          {
            // Run short Monte Carlo computation to find optimal step-size
            h_R = LocateOptimal_h_R(N_P, D, var_param, fix_value, omega, TrialWaveFunction2);
            cout << "Optimal h_R" << endl;
            cout << h_R << "\n--------------" << endl;
...
double* LocalEnergyT2U(int N_P, int D, double** &pos, double psi_curr, double h_R, double alpha, double beta, double omega,
                    double (*trialFunction)(int, int, double** &, double, double, double) )
  {
    ...
    E_Local[1] = U_Harmonic_osc(N_P, D, omega, pos);
